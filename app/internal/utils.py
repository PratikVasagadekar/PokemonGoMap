...
class parseMapData:
    ...
    def UpdatePokemonImages(self):
        self.dfMapData['Pokemon Image'] = self.dfMapData.apply(lambda row: f"https://raw.githubusercontent.com/PratikVasagadekar/PokemonGoAssets/main/Images/{row['Name']}.png", axis=1)
...
class parseRaidsData:
    ...
    def UpdateWeatherSettings(self):
        ...
        self.dfRaids['Boosted'] = self.dfRaids.apply(lambda row: "Boosted" if row['weather'] == row['Weather Boost'] and row['weather'] != "" else "Regular", axis=1)
        return self.dfRaids
